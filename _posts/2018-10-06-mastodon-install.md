---
layout: post
title:  حوه نصب Mastodon در اوبونتو
category: 
- mastodon
- learn
tags: 
...
## الزامات
    ...
    یک سرور در حال اجرا اوبونتو 16.04 است.
    یک کاربر معمولی با تنظیمات sudo در سرور شما. 

## نصب وابستگی های مورد نیاز

اولا توصیه می شود سیستم خود را با آخرین نسخه به روز کنید. شما می توانید این کار را با اجرای دستور زیر انجام دهید:
sudo apt-get update -y sudo apt-get upgrade -y

هنگامی که سیستم شما به روز می شود، سیستم خود را مجددا راه اندازی کنید و با کاربر sudo وارد شوید.

بعد، شما باید برخی از وابستگی های مورد نیاز را به سرور خود نصب کنید. شما می توانید همه آنها را با اجرای دستور زیر نصب کنید:
...
بعد، شما همچنین باید سرور redis را به سیستم خود نصب کنید. شما می توانید آن را فقط با اجرای دستور زیر نصب کنید:


`sudo apt-get install redis-server redis-tools -y`


هنگامی که تمام اجزای نصب شده است، می توانید نصب سرور Postgresql را نصب کنید.

## نصب و پیکربندی PostgreSQL

Mastodon از پایگاه داده PostgreSQL استفاده می کند، بنابراین شما باید سرور PostgreSQL و daemon را نصب کنید. شما می توانید آنها را با دستور زیر نصب کنید:
...
بعد از خروج از کاربر mastodon و دوباره وارد شوید تا اصلاح bash اعمال شود:


`exit su - mastodon git clone https://github.com/rbenv/ruby-build.git ~/.rbenv/plugins/ruby-build`

بعد، نصب روبین 2.4.1 برای mastodon با دستور زیر:
...
بعد نصب bundler برای مدیریت وابستگی ها و غیره مستند سازی گوهر را غیرفعال کنید:
...
اول، یک فایل سرویس وب با دستور زیر ایجاد کنید:
...
بعد، نمونه Mastodon را با دستور زیر شروع کنید:
...
بعد، شما باید پروکسی معکوس را با استفاده از Nginx برای دسترسی به Mastodon با نام دامنه خود تنظیم کنید.
...
ذخیره و بستن فایل، سپس Nginx را با دستور زیر دوباره راه اندازی کنید:
